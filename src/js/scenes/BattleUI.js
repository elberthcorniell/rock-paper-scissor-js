...
import {
  ActionsMenu, EnemiesMenu, HeroesMenu, Message,
} from './utils';
...
export default class BattleUI extends Phaser.Scene {
  ...
  onEnemy() {
    ...
    const action = enemyAction[Math.floor(Math.random() * 3)];
    this.battleScene.receiveSelectAction(this.lastPlayerAction, action);
  }
  ...
  onKeyInput(event) {
    ...
    if (event.code === 'ArrowUp') { this.currentMenu.moveSelectionUp(); } else if (event.code === 'ArrowDown') { this.currentMenu.moveSelectionDown(); } else if (event.code === 'Space') { this.currentMenu.confirm(); }
  }
